import Reveal from "./Reveal";

export default function Hero() {
  return (
    <section id="home">
      <div className="hero-glow"></div>
      <div className="hero-grid"></div>

      <div className="hero-content">
        <div className="hero-status">
          <span className="hero-dot"></span>
          Open to opportunities · Bengaluru, India
        </div>
        <div className="hero-role">Software Engineer · Full-Stack</div>
        <h1>
          Impana <i>B.</i>
        </h1>
        <p className="hero-bio">
          I build scalable web applications and refined user interfaces — currently
          engineering enterprise products at AT&amp;T, with a growing focus on cloud
          architecture and AI-driven features.
        </p>
        <div className="hero-btns">
          <a href="#projects" className="btn btn-solid">
            View My Work →
          </a>
          <a href="#contact" className="btn btn-outline">
            Get In Touch
          </a>
        </div>
        <div className="hero-socials">
          <a href="#" className="sl" title="LinkedIn">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
              <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6zM2 9h4v12H2z" />
              <circle cx="4" cy="4" r="2" />
            </svg>
          </a>
          <a href="#" className="sl" title="GitHub">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z" />
            </svg>
          </a>
          <a href="mailto:impana@email.com" className="sl" title="Email">
            <svg
              width="13"
              height="13"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="1.8"
            >
              <rect x="2" y="4" width="20" height="16" rx="2" />
              <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
            </svg>
          </a>
          <a href="#" className="sl" title="Instagram">
            <svg
              width="13"
              height="13"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="1.8"
            >
              <rect x="2" y="2" width="20" height="20" rx="5" />
              <circle cx="12" cy="12" r="5" />
              <circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none" />
            </svg>
          </a>
        </div>
      </div>

      <Reveal className="hero-panel">
        <div className="panel-bar">
          <span className="pd" style={{ background: "#ff5f57" }}></span>
          <span className="pd" style={{ background: "#febc2e" }}></span>
          <span className="pd" style={{ background: "#28c840" }}></span>
          <span>profile.json</span>
        </div>
        <div className="panel-body">
          <div className="pstat">
            <div className="pn">
              1<small>yr</small>
            </div>
            <div className="pl">Industry Experience</div>
          </div>
          <div className="pstat">
            <div className="pn">2</div>
            <div className="pl">Roles at AT&T</div>
          </div>
          <div className="pstat">
            <div className="pn">
              10<small>+</small>
            </div>
            <div className="pl">Technologies</div>
          </div>
          <div className="pstat">
            <div className="pn">MCA</div>
            <div className="pl">Master's in Computer Applications</div>
          </div>
        </div>
      </Reveal>
    </section>
  );
}
